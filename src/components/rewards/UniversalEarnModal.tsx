...
export const UniversalEarnModal: React.FC<UniversalEarnModalProps> = ({ isOpen, onClose }) => {
  ...
  const [mobileTab, setMobileTab] = useState<'overview' | 'ledger' | 'redeem'>('overview');
  ...
  // Staff Commission Summary & Ledger
      ...
      {isCustomer ? (
        <>
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="bg-purple-950/70 border border-purple-500/40 p-3 rounded-2xl space-y-1 shadow-inner">
              <span className="text-[9px] bg-purple-500/20 text-purple-300 font-bold px-1.5 py-0.5 rounded font-mono">
                আপনি পাচ্ছেন
              </span>
            ...
            <div className="bg-emerald-950/70 border border-emerald-500/40 p-3 rounded-2xl space-y-1 shadow-inner">
              <span className="text-[9px] bg-emerald-500/20 text-emerald-300 font-bold px-1.5 py-0.5 rounded font-mono">
                আপনার বন্ধু পাচ্ছে
              </span>
  ...
  const renderLedgerContent = () => (
    <div className="space-y-2.5">
      ...
      <div className="space-y-2 max-h-80 md:max-h-[380px] overflow-y-auto pr-1">
        {(isCustomer ? customerLedger : staffLedger).map((txn) => (
          <div 
            key={txn.id}
            className="p-3 bg-slate-950 border border-slate-800 rounded-2xl flex items-center justify-between text-xs shadow-inner hover:border-slate-700 transition"
          >
            <div className="flex items-center space-x-2.5 min-w-0 pr-2">
              ...
              <div className="min-w-0">
                <span className="font-bold text-slate-200 block truncate">{txn.title}</span>
                <span className="text-[10px] text-slate-400 block truncate mt-0.5">{txn.desc}</span>
  ...
  return (
    <>
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/85 backdrop-blur-md p-2 sm:p-4 select-none animate-in fade-in">
        {/* Full-width responsive container: Mobile fit-to-screen card, Desktop widescreen dual-column dashboard */}
        <div className="bg-slate-900 border border-purple-500/50 rounded-3xl w-full max-w-full sm:max-w-xl md:max-w-5xl lg:max-w-6xl p-4 md:p-6 shadow-2xl space-y-4 max-h-[95vh] overflow-y-auto flex flex-col justify-between">
          
          <div className="space-y-4">
            {/* Top Header Bar */}
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <div className="flex items-center space-x-3">
                <div className="w-10 h-10 md:w-12 md:h-12 rounded-2xl bg-gradient-to-tr from-purple-600 to-pink-600 text-white flex items-center justify-center shadow-lg shadow-purple-600/30 shrink-0">
                  <Gift className="w-5 h-5 md:w-6 md:h-6 animate-pulse" />
                </div>
                <div>
                  <h3 className="font-extrabold text-sm md:text-base text-white flex items-center space-x-2">
                    <span>{isCustomer ? '🎁 আর্ন ও রেফারেল রিওয়ার্ডস হাব' : '💰 সেলস ও পার্টনার কমিশন হাব'}</span>
                    <span className="text-[9.5px] bg-purple-500/20 text-purple-300 border border-purple-500/40 px-2 py-0.5 rounded-full font-mono font-bold">
                      {isCustomer ? 'EARN HUB' : 'WALLET'}
                    </span>
                  </h3>
                  <p className="text-[10px] md:text-xs text-slate-400 mt-0.5">
                    {isCustomer 
                      ? 'রেফার করুন, ক্যাশব্যাক আয় করুন ও রিয়েলটাইম লেজার হিস্ট্রি ট্র্যাক করুন' 
                      : 'প্রতি ডিভাইস অনবোর্ডিংয়ে ৳৫০০ কমিশন ও পূর্ণাঙ্গ পে-আউট লেজার'}
                  </p>
                </div>
              </div>
            ...
            <div className="grid grid-cols-3 gap-1.5 bg-slate-950 p-1 rounded-2xl border border-slate-800 md:hidden">
              <button
                type="button"
            ...
            <div className="block md:hidden">
              {mobileTab === 'overview' && renderOverviewContent()}
              {mobileTab === 'ledger' && renderLedgerContent()}
              {mobileTab === 'redeem' && renderRedeemContent()}
            </div>

            {/* DESKTOP & LAPTOP WIDESCREEN DASHBOARD VIEW (>= md) */}
            <div className="hidden md:grid md:grid-cols-12 md:gap-5">
              {/* Left Column (5 Cols): Overview, Referral Code & Share + Redeem Actions */}
              <div className="md:col-span-5 space-y-4">
                <div className="bg-slate-950/70 p-4 rounded-3xl border border-slate-800 space-y-4">
                  <div className="flex items-center space-x-2 text-purple-300 font-extrabold text-xs">
                    <Sparkles className="w-4 h-4 text-purple-400" />
                    <span>আর্নিং ও রেফারেল সামারি</span>
                  </div>
                ...
                <div className="bg-slate-950/70 p-4 rounded-3xl border border-slate-800 space-y-3">
                  <div className="flex items-center space-x-2 text-amber-300 font-extrabold text-xs">
                    ...
                    <span>ক্যাশআউট ও রিডিম অপশন</span>
                  </div>
                  {renderRedeemContent()}
                </div>
              </div>

              {/* Right Column (7 Cols): Comprehensive Interactive Ledger */}
              <div className="md:col-span-7 bg-slate-950/70 p-4 rounded-3xl border border-slate-800 flex flex-col justify-between space-y-3">
                <div>
                  <div className="flex items-center justify-between pb-2 border-b border-slate-800">
                    <div className="flex items-center space-x-2 text-emerald-300 font-extrabold text-xs">
                      <History className="w-4 h-4 text-emerald-400" />
                      <span>রিয়েলটাইম আর্নিংস ও কমিশন লেজার লগ</span>
                    </div>
                    <span className="text-[10px] font-mono text-slate-400 font-bold bg-slate-900 px-2.5 py-1 rounded-xl border border-slate-800">
                      মোট ট্রানজেকশন: {(isCustomer ? customerLedger : staffLedger).length} টি
                    </span>
                  </div>
                ...
                {/* Ledger Help / Guarantee Box */}
                <div className="p-3 rounded-2xl bg-slate-900/90 border border-slate-800 text-[11px] text-slate-400 flex items-center justify-between">
                  <span>💡 যেকোনো লেনদেন সমস্যা বা পে-আউট অনুসন্ধানের জন্য ২৪/৭ সাপোর্ট সক্রিয় রয়েছে।</span>
                  <span className="text-purple-400 font-bold font-mono">24/7 VERIFIED</span>
                </div>
              </div>
            </div>
          ...
          <div className="pt-3 border-t border-slate-800 flex items-center justify-between">
            <span className="text-[10px] md:text-xs text-slate-500 font-mono">
              EasyTracker Universal Earn Engine v2.4
            </span>

            <button
              type="button"
              onClick={onClose}
              className="px-6 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 font-bold text-xs transition active:scale-95 shadow-sm"
            >
              বন্ধ করুন (Close)
            </button>
          </div>
